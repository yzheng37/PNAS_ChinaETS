...
***** Don't consider PSM score caliper in the matching with multiple control units, due to: (1) it turns out that doesn't matter; (2) it makes the procedure much more complicated and confusing.
...
*/ matchon(industry_num MissType) sliceby(MissType) prime_id(treat_id) score scorevar(Mahscore) nummatches(`K')	// 1 to K matching
...
gen weight_con = dup*(1/`K') + 1*(1/`K')	// 1 to K matching
...
merge 1:m id_unified using MahMatch_forTreat.dta
...
// For control units
merge m:1 id_unified using MahMatch_forControl.dta
...
merge m:1 id_unified using DropPS_control.dta
...
gen DropMah = 1 if MahDist > 1 & MahDist != .
...
merge m:1 id_unified using DropMah_control.dta
...
// drop duplicates in treated units
duplicates drop id_unified if Treat == 1, force
...
save Mah_StrMod_`t'_1to`K'.dta, replace
...
use Mah_StrMod_2013_1to`K'.dta, clear

append using Mah_StrMod_2014_1to`K'.dta
...
save Mah_StrMod_1to`K'.dta, replace
...
merge m:1 id_unified using "Mah_StrMod_1to`K'.dta"	// select 1 to K
...
logout, save(Summary_All_stat_1to`K') excel replace: /*
...
logout, save(Summary_ETS_stat_1to`K') excel replace: /*
...
logout, save(Summary_NonETS_stat_1to`K') excel replace: /*
*/ tabstat logEmiss logEmissOutput logEmissSale /*
*/ logoutput logsale logElectric ElecCERatio GasCERatio /*
*/ logemploy logwage logcapital logvalueadd loginvest logexport /*
*/ TFP_OPDL logCapLab logOutputLab logOutputCap logprice turnover /*
...
estout using "DescripStat_unmatched_1to`K'.xls", replace cells("b se t p N_1 mu_1 N_2 mu_2")
...
estout using "DescripStat_matched_1to`K'.xls", replace cells("b se t p N_1 mu_1 N_2 mu_2")
...
estout using "DescripStat_treat_1to`K'.xls", replace cells("b se t p N_1 mu_1 N_2 mu_2")
...
foreach Y of varlist $dep_var_main {
	...
	outreg2 using "ChinaETSEmis_Mah_1toK.xls", excel keep(c.regu#c.post_impltPSM c.region#c.post_anncPSM) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y, Matching, 1:`K')
}

foreach Y of varlist $dep_var_main {
	quietly xi:xtreg `Y' c.regu##c.post_impltPSM c.region##c.post_anncPSM i.year i.ProTrend i.SecTrend, fe vce(cluster stderror)
	outreg2 using "ChinaETSEmis_Mah_1toK.xls", excel keep(c.regu#c.post_impltPSM c.region#c.post_anncPSM) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y, Matching, 1:`K')
}
