...
foreach v of varlist employ capital CapLab wage invest valueadd output export OutputLab OutputCap {
	gen log`v' = log(`v') 
}
...
*/ tabstat logEmiss logEmissOutput logCE logCEOutput /*
*/ logoutput logsale logemploy logwage logWageLab logcapital logvalueadd logexport loginvest TFP_OP logCapLab logOutputLab logOutputCap logprice turnover if Match == 1, stat(count mean sd min max) long f(%6.3f) c(s)

logout, save(Summary_ETS_stat) excel replace: /*
*/ tabstat logEmiss logEmissOutput logCE logCEOutput /*
*/ logoutput logsale logemploy logwage logWageLab logcapital logvalueadd logexport loginvest TFP_OP logCapLab logOutputLab logOutputCap logprice turnover if regu == 1 & Match == 1, stat(count mean sd min max) long f(%6.3f) c(s)

logout, save(Summary_NonETS_stat) excel replace: /*
*/ tabstat logEmiss logEmissOutput logCE logCEOutput /*
*/ logoutput logsale logemploy logwage logWageLab logcapital logvalueadd logexport loginvest TFP_OP logCapLab logOutputLab logOutputCap logprice turnover if regu == 0 & Match == 1, stat(count mean sd min max) long f(%6.3f) c(s)
...
keep id_unified year regu Match logEmiss logEmissOutput logCE logoutput logsale logCEOutput logemploy logwage logcapital logvalueadd loginvest

keep if year == 2009 | year == 2010

reshape wide logEmiss logEmissOutput logCE logoutput logsale logCEOutput logemploy logwage logcapital logvalueadd loginvest, i(id_unified regu Match) j(year)


// Unmatched samples
estpost ttest logEmiss2010 logEmissOutput2010 logCE2010 logoutput2010 logsale2010 logCEOutput2010 logemploy2010 logwage2010 logcapital2010 logvalueadd2010 loginvest2010 logEmiss2009 logEmissOutput2009 logCE2009 logoutput2009 logsale2009 logCEOutput2009 logemploy2009 logwage2009 logcapital2009 logvalueadd2009 loginvest2009, by(regu)

estout using "Balance_unmatched.xls", replace cells("b se t p N_1 mu_1 N_2 mu_2")


// Matched samples
estpost ttest logEmiss2010 logEmissOutput2010 logCE2010 logoutput2010 logsale2010 logCEOutput2010 logemploy2010 logwage2010 logcapital2010 logvalueadd2010 loginvest2010 logEmiss2009 logEmissOutput2009 logCE2009 logoutput2009 logsale2009 logCEOutput2009 logemploy2009 logwage2009 logcapital2009 logvalueadd2009 loginvest2009 if Match == 1, by(regu)

estout using "Balance_matched.xls", replace cells("b se t p N_1 mu_1 N_2 mu_2")
...
foreach Y of varlist $dep_var_main {
	...
	outreg2 using "Baseline.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}


** The Channels of Carbon Emission Reductions
foreach Y of varlist $dep_var_abate {
	quietly reghdfe `Y' c.regu##c.post_implt c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Channel.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}


** The ETS Effects on Firm Economic Attributes
foreach Y of varlist $dep_var_econ {
	quietly reghdfe `Y' c.regu##c.post_implt c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "EconAttr.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}



** The ETS Effects of Carbon Price and Allowance Liquidity
foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' logprice c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "MarketPerform.xls", excel keep(logprice c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}

foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' turnover c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "MarketPerform.xls", excel keep(turnover c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}
...
foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.post_implt##c.regu##c.RateBaseGroup c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "RateBase.xls", excel keep(c.post_implt#c.regu c.post_implt#c.regu#c.RateBaseGroup c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}
