...
global covariate `x1'2009_NoMiss `x1'2010_NoMiss `x2'2009_NoMiss `x2'2010_NoMiss `x3'2009_NoMiss `x3'2010_NoMiss `x4'2009_NoMiss `x4'2010_NoMiss
...
save MatchMahAltCov_`t'.dta, replace
...
use MatchMahAltCov_2013.dta, clear

append using MatchMahAltCov_2014.dta
...
save MatchMahAltCov.dta, replace
...
merge m:1 id_unified using "MatchMahAltCov.dta"
...
* Robustness Checks on Alternative Sets of Covariates
foreach Y of varlist $dep_var_main {
	...
	outreg2 using "Robust_AltCovSet.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
...
foreach Y of varlist $dep_var_main {
	...
	outreg2 using "Robust_AltCovSet.xls", excel keep(c.post_implt#c.regu c.post_implt#c.regu#c.RateBaseGroup c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}
