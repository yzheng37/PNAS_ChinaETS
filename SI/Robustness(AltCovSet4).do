...
foreach Y of varlist $dep_var_main {
	...
	outreg2 using "ChinaETSEmis_Mah_othercov.xls", excel keep(c.regu#c.post_impltPSM c.region#c.post_anncPSM) dec(3) /*
...
foreach Y of varlist $dep_var_main {
	...
	*/ , absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "ChinaETSEmis_Mah_Rate_othercov.xls", excel keep(c.post_impltPSM#c.regu c.post_impltPSM#c.regu#c.RateBaseGroup c.region#c.post_anncPSM) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}
