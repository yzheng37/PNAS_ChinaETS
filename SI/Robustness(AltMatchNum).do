...
save MatchMah_`t'_1to`K'.dta, replace
...
use MatchMah_2013_1to`K'.dta, clear

append using MatchMah_2014_1to`K'.dta
...
save MatchMah_1to`K'.dta, replace
...
merge m:1 id_unified using "MatchMah_1to`K'.dta" // select K
...
expand 2 if (Weight2013 == (1/`K')*2 | Weight2014 == (1/`K')*2) & regu == 0, gen(Dup_MultiUnit2)
expand 3 if (Weight2013 == (1/`K')*3 | Weight2014 == (1/`K')*3) & regu == 0, gen(Dup_MultiUnit3)
...
* Robustness Checks on Alternative Matching Numbers
foreach Y of varlist $dep_var_main {
	...
	outreg2 using "Robust_AltMatchNum.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
