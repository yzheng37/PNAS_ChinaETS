...
* Robustness Checks on Alternative Emission Measurements
// exclude sectors with significant emissions from industrial process
** iron and steel
...
** cement, lime, glass and other building materials sectors
replace IndProcEmiss = 1 if strmatch(IND2_name, "*非金属矿物*")


foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.regu##c.post_implt c.region##c.post_annc if Match == 1 & IndProcEmiss != 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_AltMeas.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y, DropIndProcEmiss, Y)
}


foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.post_implt##c.regu##c.RateBaseGroup c.region##c.post_annc if Match == 1 & IndProcEmiss != 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_AltMeas.xls", excel keep(c.post_implt#c.regu c.post_implt#c.regu#c.RateBaseGroup c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y, DropIndProcEmiss, Y)
}
...
* Robustness Checks on Confounding Factors

** Additional FE
foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.regu##c.post_implt if Match == 1, absorb(id year ProYear SecYear) vce(cluster stderror)
	outreg2 using "Robust_AddFE.xls", excel keep(c.regu#c.post_implt) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year FE, Y, Industry-Year FE, Y)
}

foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.post_implt##c.regu##c.RateBaseGroup if Match == 1, absorb(id year ProYear SecYear) vce(cluster stderror)
	outreg2 using "Robust_AddFE.xls", excel keep(c.post_implt#c.regu c.post_implt#c.regu#c.RateBaseGroup) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year FE, Y, Industry-Year FE, Y)
}
...
** Without BTH
foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.regu##c.post_implt c.region##c.post_annc if Match == 1 & province != "北京市" & province != "天津市" & province != "河北省", absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_noBTH.xls", excel keep(c.regu#c.post_implt c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y, DropBTH, Y)
}

foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.post_implt##c.regu##c.RateBaseGroup c.region##c.post_annc if Match == 1 & province != "北京市" & province != "天津市" & province != "河北省", absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_noBTH.xls", excel keep(c.post_implt#c.regu c.post_implt#c.regu#c.RateBaseGroup c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y, DropBTH, Y)
}


** Top-10k
gen post_ES10k = 1 if post_implt != . & year >= 2012
replace post_ES10k = 0 if post_implt != . & post_ES10k == .

foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.regu##c.post_implt c.ES10k##c.post_ES10k c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_Top10k.xls", excel keep(c.regu#c.post_implt c.ES10k#c.post_ES10k c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}

foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.post_implt##c.regu##c.RateBaseGroup c.post_ES10k##c.ES10k c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_Top10k.xls", excel keep(c.post_implt#c.regu c.post_implt#c.regu#c.RateBaseGroup c.post_ES10k#c.ES10k c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}
