...
* Robustness: Policy Heterogeneity based upon Carbon Market Performance
** (1) carbon price
foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.logprice##c.RateBaseGroup c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_HeteroPolicy.xls", excel keep(logprice c.logprice#c.RateBaseGroup c.region#c.post_annc) dec(3) /*
...
** (2) turnover rate
foreach Y of varlist $dep_var_main {
	quietly reghdfe `Y' c.turnover##c.RateBaseGroup c.region##c.post_annc if Match == 1, absorb(id year ProTrend SecTrend) vce(cluster stderror)
	outreg2 using "Robust_HeteroPolicy.xls", excel keep(turnover c.turnover#c.RateBaseGroup c.region#c.post_annc) dec(3) /*
	*/ addtext(Firm FE, Y, Year FE, Y, Province-Year linear trend, Y, Industry-Year linear trend, Y)
}
